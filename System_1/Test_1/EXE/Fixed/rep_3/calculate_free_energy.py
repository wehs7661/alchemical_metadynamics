...
def preprocess_data(folders, temp, bounds=[None, None]):
    """
    ...
    for i in range(len(folders)):
        ...
        truncated_dHdl = equilibrium_detection(dHdl, dHdl.iloc[:, 0], lower=bounds[0], upper=bounds[1])
        truncated_u_nk = equilibrium_detection(u_nk, u_nk.iloc[:, 0], lower=bounds[0], upper=bounds[1])
...
if __name__ == "__main__":
    ...
    if os.path.isfile('dHdl_u_nk_data.pickle') is True:
        ...
        dHdl, u_nk = preprocess_data(folders, args.temp, args.bounds)
