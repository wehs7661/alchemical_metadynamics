...
if __name__ == "__main__":
    ...
    if args.shift is not None:
        ...
        f_shift = list(data_shift['file.free'] / factor) 
    ...
    if args.shift is not None:
        ...
        ax1.plot_trisurf(CV_1, CV_2, f, cmap='viridis', edgecolor='none') 
    ax1.set_xlabel(args.xlabel)  # color='white'
    ax1.set_ylabel(args.ylabel)  # color='white'
    ax1.set_zlabel('Free energy ($ k_{B}T $)')  # color='white'
    ...
    if args.contour is True:
        CV1_grids = np.linspace(np.min(CV_1), np.max(CV_1), int(np.ptp(CV_1) / dx_list[0]) + 1) / args.factors[0]
    ...
    if args.CV[0] == 'lambda':
        ...
        if args.contour is True:
            ax2.set_xticks(np.arange(0, n, int(space/2)))
    if args.CV[1] == 'lambda':
        n = int(max(CV_2)) + 1
        space = int(np.ceil(n / 10))
        ax1.set_yticks(np.arange(0, n + 1, space))
        if args.contour is True:
            ax2.set_yticks(np.arange(0, n, int(space/2)))

    if args.range_x is not None:
        ax1.set_xlim(args.range_x)
        if args.contour is True:
            ax2.set_xlim(args.range_x)
    if args.range_y is not None:
        ax1.set_ylim(args.range_y)
        if args.contour is True:
            ax2.set_ylim(args.range_y)
